...
interface Provider {
  ...
  verification_status: 'pending' | 'approved' | 'rejected' | 'unverified';
...
export default function ProvidersPage() {
  ...
  const loadProviders = async () => {
    setLoading(true);
    try {
      const data = await fetchAPI<{ items: Provider[], total: number }>('/api/v1/admin/providers');
  ...
  const filteredProviders = providers.filter(p =>
    ...
    p.owner?.phone?.toLowerCase().includes(searchQuery.toLowerCase())
  );
          ...
          <div key={p.id} className="group bg-white rounded-[32px] p-8 border border-black/5 hover:border-black/10 transition-all shadow-sm flex flex-col h-full relative overflow-hidden">
            ...
            <div className="flex items-start justify-between mb-8 relative z-10">
              ...
              <div className="flex flex-col gap-2 items-end">
                <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${p.verification_status === 'approved' ? 'text-blue-600 bg-blue-500/10' : 'text-orange-600 bg-orange-500/10'
                  }`}>
                  {p.verification_status === 'approved' ? <ShieldCheck className="inline-block mr-1" size={12} /> : null}
            ...
            <div className="flex-1 relative z-10">
              <h3 className="text-xl font-black tracking-tighter uppercase mb-2 group-hover:text-blue-600 transition-colors">
                {p.owner?.full_name || 'N/A'}
              </h3>
              <p className="text-[10px] font-black uppercase tracking-widest text-black/30 mb-4 font-mono">{p.provider_type}</p>

              <div className="space-y-3">
                <div className="flex items-center gap-3 text-black/40 text-xs font-bold">
                  <Phone size={14} className="shrink-0" />
                  <span>{p.owner?.phone || 'N/A'}</span>
                </div>
                <div className="flex items-center gap-3 text-black/40 text-xs font-bold">
                  <CheckCircle2 size={14} className="shrink-0 text-blue-500" />
                  <span>{p.total_jobs_completed} hoàn thành</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-10 pt-8 border-t border-black/5 flex items-center justify-between relative z-10">
              <button
                onClick={() => { setSelectedProvider(p); setIsDetailModalOpen(true); }}
                className="text-[10px] font-black uppercase tracking-widest text-black hover:text-blue-600 flex items-center gap-1.5 transition-colors"
              >
                Chi tiết hồ sơ <ExternalLink size={12} strokeWidth={3} />
              </button>
              <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${p.status === 'active' ? 'text-emerald-700 bg-emerald-500/10' : 'text-black/20 bg-black/5'
                }`}>
                {p.status === 'active' ? <CheckCircle2 size={12} strokeWidth={3} /> : <AlertCircle size={12} strokeWidth={3} />}
                {p.status}
              </div>
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-10">
                <div className="flex gap-6 items-center">
                  ...
                  <div className="flex flex-col">
                    <h2 className="text-3xl font-black uppercase tracking-tighter leading-none mb-2">{selectedProvider.owner.full_name}</h2>
                    <span className="text-black/40 text-[10px] font-black uppercase tracking-widest font-mono">PID: {selectedProvider.id}</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div className="col-span-2 p-6 bg-black/[0.02] rounded-[24px] border border-black/5">
                  <p className="text-[10px] font-black uppercase tracking-widest text-black/30 mb-2">Giới thiệu dịch vụ</p>
                  <p className="text-sm font-bold leading-relaxed">{selectedProvider.description || 'Chưa có mô tả chi tiết cho nhà cung cấp này.'}</p>
                </div>

                <div className="space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-black/30 border-b border-black/5 pb-2">Thông tin liên hệ</p>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <Phone size={16} className="text-black/20" />
                      <span className="text-sm font-bold font-mono">{selectedProvider.owner.phone}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <MapPin size={16} className="text-black/20" />
                      <span className="text-sm font-bold text-black/40 italic">Chưa đăng ký địa chỉ</span>
                    </div>
                  </div>
                </div>

                <div className="space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-black/30 border-b border-black/5 pb-2">Hiệu suất</p>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <Sparkles size={16} className="text-orange-500" />
                      <span className="text-sm font-black uppercase tracking-widest">{selectedProvider.avg_rating} Sao</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <CheckCircle2 size={16} className="text-emerald-500" />
                      <span className="text-sm font-black uppercase tracking-widest">{selectedProvider.total_jobs_completed} Việc đã xong</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-12 flex gap-4">
                <button className="flex-1 py-4 bg-black text-white rounded-[24px] font-black text-[11px] uppercase tracking-widest hover:scale-[1.02] transition-all">
                  Duyệt / Từ chối hồ sơ
                </button>
                <button
                  onClick={() => setIsDetailModalOpen(false)}
