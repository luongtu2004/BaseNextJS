...
| Phase 9 — Rating, Notifications & Analytics | Lên kế hoạch | — | — | 2-way reviews, WS notifications (no FCM), admin dashboard. |
...
| 12 | WS notification (booking events) | 9.2 + 11 | Tài xế nhận chuyến, khách nhận thông báo qua WebSocket |
...
| **S3** | 5–6 | Booking flow đầy đủ (tiền mặt) + OTP lên xe + WS notification | Đặt và hoàn thành được chuyến |
...
> Khi `pricing_mode = driver_quote`: tài xế nhận yêu cầu, xem km và điểm đi/đến, tự nhập giá và gửi về. Khách nhận thông báo qua WebSocket, xem giá + km, chọn **Đồng ý** hoặc **Từ chối**. Nếu từ chối → hệ thống dispatch tài xế khác.
...
> Khi tài xế chuyển sang `arrived`, hệ thống sinh mã OTP 4 số gửi thông báo qua WebSocket về app khách.  
...
**Mục tiêu:** Hoàn thiện trải nghiệm sau chuyến — đánh giá 2 chiều, thông báo in-app (WS real-time delivery, không dùng Firebase/FCM), dashboard analytics cho admin.  
**Bảng DB mới:** `reviews`, `notifications`, `notification_settings`

> **Notification Delivery Model (WS-only):**  
> - Phase 9 tạo **DB persistence + REST API** cho notifications.  
> - Phase 11 (WS Server) sẽ PUBLISH qua Redis → broadcast đến client đang connected.  
> - Khi user không có WS connection (app tắt), notification chỉ lưu DB — user thấy khi mở app lại (pull model qua REST).  
> - **Không dùng Firebase/FCM** — toàn bộ notification và chat đều đi qua WebSocket.
...
| # | Method | Path | Notes | Trạng thái |
|---|--------|------|-------|------------|
| RV1 | POST | `/api/v1/customer/bookings/{id}/review` | Đánh giá tài xế sau chuyến (booking phải `completed`) | TODO |
| RV2 | GET | `/api/v1/customer/providers/{id}/reviews` | Xem đánh giá của provider (paginated) | TODO |
...
| PRV2 | GET | `/api/v1/provider/me/reviews` | Xem đánh giá mình nhận được (paginated) | TODO |

#### MODULE A — Admin Reviews

| # | Method | Path | Notes | Trạng thái |
|---|--------|------|-------|------------|
| ARV1 | GET | `/api/v1/admin/reviews` | Toàn bộ đánh giá + filter (reviewer_role, rating, is_visible) | TODO |
| ARV2 | PATCH | `/api/v1/admin/reviews/{id}/visibility` | Ẩn/hiện đánh giá vi phạm | TODO |

---

### 9.2 — Notifications (WS-only, no Firebase)
...
CREATE TABLE notifications (
    ...
    -- admin_broadcast / promotion / system
    title           VARCHAR(200) NOT NULL,
    body            TEXT,
    data            JSONB,       -- deep link, booking_id, review_id, etc.
...
> **Delivery:** Phase 9 chỉ lưu DB + REST query. Phase 11 (WS Server) sẽ thêm PUBLISH Redis → broadcast đến client connected.  
> **Không dùng FCM** — user offline sẽ thấy notification khi mở app lại.

#### MODULE — Notifications (Common cho cả Customer và Driver)

| # | Method | Path | Notes | Trạng thái |
|---|--------|------|-------|------------|
| NF1 | GET | `/api/v1/common/me/notifications` | Danh sách thông báo (paginated) | TODO |
...
| NF5 | GET | `/api/v1/common/me/notification-settings` | Xem cấu hình nhận thông báo | TODO |
| NF6 | PUT | `/api/v1/common/me/notification-settings` | Cập nhật on/off từng loại notification | TODO |

#### MODULE A — Admin Notification

| # | Method | Path | Notes | Trạng thái |
|---|--------|------|-------|------------|
| ANF1 | POST | `/api/v1/admin/notifications/broadcast` | Gửi thông báo hàng loạt (lưu DB, WS delivery ở Phase 11) | TODO |
...
│   (stateless, scale ∞)     │ │   /ws/notifications        │
│                             │ │   (stateful, persistent)   │
└───────────┬────────────┘ └───────────┬────────────┘
            │                           │
            └───────────┬───────────┘
...
POST /customer/bookings
  ...
  → Gửi WS notification đến driver (PUBLISH Redis → WS broadcast)
  → Đồng thời INSERT notification record vào DB
...
FastAPI: PATCH /provider/bookings/{id}/status → arriving
  ...
  → INSERT notification record (DB persistence)
  ...
  → Nếu client không connected → notification lưu DB, user thấy khi mở app lại
```
...
ws-server/
  app/
    ...
    services/
      ...
      notification_relay.py  # Relay notification từ REST → WS broadcast
    models/
...
**Offline handling (WS-only, no FCM):**
- Nếu user không có WS connection → notification + message đều lưu DB
- Khi user mở app lại → pull via REST API (`GET /notifications`, `GET /messages`)
- App hiển thị badge count từ `GET /notifications/unread-count`
- Chat message: lưu DB trước → nếu WS connected thì broadcast ngay, nếu không thì user thấy khi mở app
